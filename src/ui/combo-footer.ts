...
import type { ComboboxController } from './combobox.js';

/** `attachComboFooter`'s options bag. Only `isOpen()` off the real
 *  `ComboboxController` is ever read here. */
export interface AttachComboFooterOpts {
  listEl: HTMLElement;
  combo: Pick<ComboboxController, 'isOpen'>;
  hasRecents: () => boolean;
  onClear: () => void;
}

export interface ComboFooter {
  el: HTMLElement;
  sync: () => void;
}

export function attachComboFooter({ listEl, combo, hasRecents, onClear }: AttachComboFooterOpts): ComboFooter {
  const btn = h('button', {
  ...
  function sync(): void {
    ...
    // cast instead (see combobox.ts's `position()` for the same call).
    const pos = fixedAnchor(rect, { gap: 0 }) as { top: number; left: number };
