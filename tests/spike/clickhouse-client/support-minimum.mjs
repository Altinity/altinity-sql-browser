...
const SCAN_TARGETS = [
  ...
  {
    label: 'docs/ (deployment/demo/operations docs; excluding docs/design — a hypothetical, not-yet-built feature design tree, see docs/design/google-sheets/README.md\'s own "design-only" framing — its version mentions describe a FUTURE feature\'s own configurable minimum, not a current repository promise; excluding docs/evidence — this scan\'s OWN previously-committed evidence output tree (e.g. this very support-minimum-analysis.md and results.json), which is a record of what this spike found, never a repository version promise/pin to be discovered — scanning it re-ingests the scan\'s own prior output on every regeneration, ballooning findings with self-quotation without changing the derived minimum; and excluding docs/ADR-0005-clickhouse-web-client.md itself — the SAME self-reference one level up: the ADR\'s Decision/support-minimum-derivation sections are AUTHORED FROM this scan\'s own conclusions (plan §30 requires the ADR to quote the support-minimum derivation and decision table verbatim), so once the ADR exists at HEAD, re-running this scan would otherwise discover the ADR quoting "ClickHouse 24.8"/"26.3.16.10001.altinitystable" back at itself and misreport them as a pre-existing repository promise, when they are the scan\'s own output narrated in prose form)',
    globDir: 'docs',
    extensions: ['.md', '.xml'],
    excludeDirs: ['docs/design', 'docs/evidence'],
    excludeFiles: ['docs/ADR-0005-clickhouse-web-client.md'],
  },
...
function walk(dirAbs, extensions, excludeAbsList, out) {
  let entries;
  try {
  ...
  for (const entry of entries) {
    ...
    if (excludeAbsList && excludeAbsList.includes(abs)) continue; // matches an excluded dir OR an excluded individual file
    if (entry.isDirectory()) {
      if (entry.name === 'node_modules' || entry.name === '.git') continue;
      walk(abs, extensions, excludeAbsList, out);
...
export function scanForPinnedServerVersion(repoRoot = DEFAULT_REPO_ROOT) {
  ...
  for (const target of SCAN_TARGETS) {
    ...
    const excludeRelList = [
      ...(target.excludeDirs || (target.excludeDir ? [target.excludeDir] : [])),
      ...(target.excludeFiles || []),
    ];
    const excludeAbsList = excludeRelList.map((d) => join(repoRoot, d));
    walk(dirAbs, target.extensions, excludeAbsList, files);
  }
...
// ── docs/evidence/585/support-minimum-analysis.md rendering (plan §13) ──────
// Moved here (from run-matrix.mjs) so it lives next to the data it renders —
// `deriveProposedMinimum()`'s own return shape and `SERVER_SENSITIVE_INVENTORY`
// — and so a one-off evidence-only regeneration can import just this module
// (pure, side-effect-free) without importing run-matrix.mjs itself, whose
// `main()` runs unconditionally on import (it boots Docker/browsers).

export function renderSupportMinimumMd(analysis) {
  const L = [];
