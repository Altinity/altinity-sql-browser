...
// The mounted `.cm-editor` DOM node is always present once `createCodeViewer`
// has run — an erased assertion, not a runtime fallback.
function editorDom(parent: Element): HTMLElement {
  return parent.querySelector('.cm-editor') as HTMLElement;
}

function mounted(over: Partial<CodeViewerOptions> = {}) {
  ...
  // The viewer always mounts a real `.cm-editor` node synchronously — an
  // erased assertion, not a runtime fallback.
  const view = EditorView.findFromDOM(editorDom(parent))!;
...
describe('read-only code viewer', () => {
  ...
  it('installs the local Mod-f search keymap and keeps the panel inside the viewer', () => {
    ...
    const event = (modifier: 'ctrlKey' | 'metaKey') => new KeyboardEvent('keydown', {
  ...
  it('toggles wrapping through a compartment without rebuilding the view', () => {
    ...
    viewer.setWrap(true);
    expect(EditorView.findFromDOM(editorDom(parent))).toBe(view);
  ...
  it('loads JSON, SQL, XML, and XML-style HTML, then reconfigures language in place', () => {
    const cases: [CodeViewerOptions['language'], string, string][] = [
