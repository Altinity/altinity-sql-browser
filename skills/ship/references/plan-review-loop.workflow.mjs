...
for (let pass = startPass; pass <= 5; pass++) {
  ...
  // A generic output filename is reused across every unit's plan review for an ENTIRE /ship
  // run that can span many hours — observed live on #630 phase 6's code-review loop (the
  // analogous PR-review case): a review-runner agent's structured-output step reported a
  // stale file's content (a different unit's old, already-resolved review) as this pass's
  // real result. Naming the file after this exact plan file + pass avoids that collision.
  const planSlug = runArgs.planFile.split('/').pop().replace(/\.[^.]+$/, '')
  const outputFile = `$TMPDIR/chatgpt-review-plan-${planSlug}-pass${pass}.json`
