...
import { scanSpans } from '../../src/index.js';
...
function spans(text?: string | null): [string, string][] {
  ...
  return list.map(({ kind, start, end }): [string, string] => [kind, s.slice(start, end)]);
}

// The raw spans with their `closed` flags, for the recovery-policy assertions.
const raw = (text?: string | null): [string, boolean][] =>
  [...scanSpans(text)].map(({ kind, closed }): [string, boolean] => [kind, closed]);
