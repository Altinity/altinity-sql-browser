...
import type { SerializeParamResult } from '../../src/core/param-serialize.js';
import { parseParamType } from '../../src/core/param-type.js';

// Narrows a result expected to be the error variant — throws (failing the
// test) if the implementation regressed to `ok: true`, same signal
// `expect(r.ok).toBe(false)` already gives, just with the type-checker
// narrowing `r.error`/`r.structural` for the assertions that follow.
function asErr(r: SerializeParamResult): { ok: false; error: string; structural?: boolean } {
...
describe('serializeParamValue — scalar passthrough (byte-identical with today)', () => {
  ...
  it('rejects an unsupported value type with a clear error', () => {
    const r = asErr(serializeParamValue({ nope: 1 }, 'String', 'x'));
...
describe('serializeParamValue — Array(T) literals', () => {
  ...
  it('rejects a non-integer token for an integer element type', () => {
    const r = asErr(serializeParamValue(['1', 'abc'], 'Array(UInt64)', 'ids'));
    expect(r.ok).toBe(false);
  ...
  it('rejects a leading-zero integer element (007) with a clear error — the live-verified grammar the scalar path already enforces', () => {
    const r = asErr(serializeParamValue(['007'], 'Array(UInt64)', 'ids'));
    expect(r.ok).toBe(false);
...
describe('serializeParamValue — rejections', () => {
  it('array value against a scalar declaration is a structural error', () => {
    const r = asErr(serializeParamValue(['a'], 'String', 'db'));
    expect(r.ok).toBe(false);
  ...
  it('NULL elements are rejected in v1, even under Array(Nullable(T))', () => {
    const r = asErr(serializeParamValue(['a', null], 'Array(Nullable(String))', 'xs'));
    expect(r.ok).toBe(false);
  ...
  it('nested arrays are rejected with a clear error (declaration and value)', () => {
    const decl = asErr(serializeParamValue([[1, 2]], 'Array(Array(UInt8))', 'm'));
    expect(decl.ok).toBe(false);
    expect(decl.error).toContain('nested Array');
    const val = asErr(serializeParamValue([['a']], 'Array(String)', 'm'));
