...
function unescapeDot(s: string): string {
  // Labels may carry escaped quotes and `\n` line breaks; collapse to one line.
  return String(s).replace(/\\n/g, ' ').replace(/\\(.)/g, '$1').trim();
}

/** One parsed DOT node — a processor id and its (unescaped) label. */
export interface DotNode {
  id: string;
  label: string;
}

/** One parsed DOT edge between two already-declared node ids. */
export interface DotEdge {
  from: string;
  to: string;
}

/** `parseDot`'s return shape — the same shape `dot-layout.js`'s `dagreLayout`/
 *  `schemaLayout` (and their richer schema-graph callers) accept as input. */
 ...
 */
export function parseDot(text: unknown): DotGraph {
