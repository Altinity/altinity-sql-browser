...
const ms = (y: number, mo: number, d: number, h = 0, mi = 0, s = 0, msec = 0): number =>
  new Date(y, mo, d, h, mi, s, msec).getTime();

// Every `resolveInstant` call in this file feeds it a genuinely-parsed
// (non-null, non-error) expression — this local narrowing helper keeps
// `resolveInstant`'s param type strict (a real caller always pre-validates,
// see `resolveRelativeValue`/`formatPreview`) without repeating the cast at
// every call site below.
const parsedExpr = (expr: string): ParsedRelativeExpr => parseRelativeExpr(expr) as ParsedRelativeExpr;
...
describe('resolveInstant — offset/rounding order and calendar arithmetic', () => {
  it('offset-then-round: "now-1h/h" rounds the offset instant, not "now"', () => {
    ...
    expect(resolveInstant(parsedExpr('now-1h/h'), now)).toBe(ms(2026, 6, 11, 13, 0, 0));
  });
  it('rounding every unit snaps DOWN', () => {
    const now = ms(2026, 6, 8, 15, 42, 17, 123); // 2026-07-08 is a Wednesday
    expect(resolveInstant(parsedExpr('now/s'), now)).toBe(ms(2026, 6, 8, 15, 42, 17, 0));
    expect(resolveInstant(parsedExpr('now/m'), now)).toBe(ms(2026, 6, 8, 15, 42, 0, 0));
    expect(resolveInstant(parsedExpr('now/h'), now)).toBe(ms(2026, 6, 8, 15, 0, 0, 0));
    expect(resolveInstant(parsedExpr('now/d'), now)).toBe(ms(2026, 6, 8, 0, 0, 0, 0));
    expect(resolveInstant(parsedExpr('now/M'), now)).toBe(ms(2026, 6, 1, 0, 0, 0, 0));
    expect(resolveInstant(parsedExpr('now/y'), now)).toBe(ms(2026, 0, 1, 0, 0, 0, 0));
  });
  it('ISO-Monday week rounding: a Wednesday rounds back to that week\'s Monday', () => {
    const wed = ms(2026, 6, 8, 15, 0, 0, 0); // Wed
    expect(resolveInstant(parsedExpr('now/w'), wed)).toBe(ms(2026, 6, 6, 0, 0, 0, 0)); // Mon
  });
  it('ISO-Monday week rounding: a Sunday rounds back to the PRECEDING Monday (not itself)', () => {
    const sun = ms(2026, 6, 12, 10, 0, 0, 0); // Sun
    expect(resolveInstant(parsedExpr('now/w'), sun)).toBe(ms(2026, 6, 6, 0, 0, 0, 0));
  });
  it('ISO-Monday week rounding: a Monday rounds to itself at midnight', () => {
    const mon = ms(2026, 6, 6, 10, 0, 0, 0);
    expect(resolveInstant(parsedExpr('now/w'), mon)).toBe(ms(2026, 6, 6, 0, 0, 0, 0));
  });
  it('month-end clamping: Mar 31 -1M → Feb 28 (2026 is not a leap year)', () => {
    const now = ms(2026, 2, 31, 10, 0, 0, 0);
    expect(resolveInstant(parsedExpr('-1M'), now)).toBe(ms(2026, 1, 28, 10, 0, 0, 0));
  });
  it('month-end clamping: leap Feb 29 -1y → Feb 28 the following (non-leap) year', () => {
    ...
    expect(resolveInstant(parsedExpr('+1y'), leapFeb29)).toBe(ms(2029, 1, 28, 10, 0, 0, 0));
  });
  it('month offset rolls the year over cleanly (no clamping needed)', () => {
    const now = ms(2026, 0, 15, 9, 0, 0, 0); // Jan 15
    expect(resolveInstant(parsedExpr('-1M'), now)).toBe(ms(2025, 11, 15, 9, 0, 0, 0)); // Dec 15, 2025
  });
  it('week offset (multiple weeks)', () => {
    const now = ms(2026, 6, 15, 9, 0, 0, 0);
    expect(resolveInstant(parsedExpr('-2w'), now)).toBe(ms(2026, 6, 1, 9, 0, 0, 0));
  });
  ...
  describe('DST spring-forward (America/New_York, 2026-03-08 02:00→03:00)', () => {
    ...
    it('"-1h" is a FIXED duration: exactly 3,600,000 ms earlier on the epoch timeline', () => {
      expect(resolveInstant(parsedExpr('-1h'), afterSpring)).toBe(afterSpring - 3600000);
    });
    it('"-1d" keeps the same wall-clock time yesterday (23 elapsed hours, not 24)', () => {
      const target = resolveInstant(parsedExpr('-1d'), afterSpring);
    ...
    it('"now/d" is local midnight even on the transition day', () => {
      expect(resolveInstant(parsedExpr('now/d'), afterSpring)).toBe(ms(2026, 2, 8, 0, 0, 0, 0));
    });
    it('"-1d/d" is local midnight of the day before the transition', () => {
      expect(resolveInstant(parsedExpr('-1d/d'), afterSpring)).toBe(ms(2026, 2, 7, 0, 0, 0, 0));
    });
  });
  ...
  describe('DST fall-back (America/New_York, 2026-11-01 02:00→01:00)', () => {
    ...
    it('"-1h" is a FIXED duration: exactly 3,600,000 ms earlier', () => {
      expect(resolveInstant(parsedExpr('-1h'), afterFallback)).toBe(afterFallback - 3600000);
    });
    it('"-1d" keeps the same wall-clock time yesterday (25 elapsed hours, not 24)', () => {
      const target = resolveInstant(parsedExpr('-1d'), afterFallback);
    ...
    it('"now/d" is local midnight on the transition day', () => {
      expect(resolveInstant(parsedExpr('now/d'), afterFallback)).toBe(ms(2026, 10, 1, 0, 0, 0, 0));
    });
  });
});
...
describe('isDateLikeType', () => {
  it('true for Date/Date32/DateTime/DateTime64, string or parsed form', () => {
    ...
    expect(isDateLikeType({ base: 'DateTime' } as ParsedParamType)).toBe(true);
  });
