...
// The brand logomark is a supplied asset with its own fixed gradient palette; it
// lives in its own module so the design-system colour detector can exclude that
// one file rather than every icon here. See src/ui/brand-logo.ts.
import { brandMark } from './brand-logo.js';

// Shared stroke attributes for the outlined icons.
const stroked = (stroke: number): Record<string, string | number> => ({
  stroke: 'currentColor', 'stroke-width': stroke,
  'stroke-linecap': 'round', 'stroke-linejoin': 'round',
});

/** `svg`'s optional 4th-argument overrides — stroke width / fill. */
...
export function svg(d: string, w = 12, hgt = 12, opts: SvgOpts = {}): SVGElement {
...
export function svgArt(body: string, w: number, h: number, vbW = w, vbH = h): SVGElement {
...
export const Icon = {
  ...
  // The Dashboard tile head's overflow trigger (#544). Introduced for a tree row's
  // trailing menu in #426, left with no caller when #494 replaced those menus with
  // direct controls, and adopted by the tile head — where five controls did not fit
  // beside a title. Deliberately NOT a chevron: a disclosure chevron means "expand
  // in place", which is a different promise from "more actions".
  ...
  dashboard: () => iconEl('<rect x="1.5" y="1.5" width="4" height="4" rx=".5"/><rect x="6.5" y="1.5" width="4" height="4" rx=".5"/><rect x="1.5" y="6.5" width="4" height="4" rx=".5"/><rect x="6.5" y="6.5" width="4" height="4" rx=".5"/>', 12, 12, 1.2),
  ...
  // open book), distinct from `shortcuts` (a keyboard glyph) above.
  book: () => iconEl('<path d="M6 2.8C5 1.9 3.5 1.6 2 1.8v7.6c1.5-.2 3 .1 4 1zM6 2.8c1-.9 2.5-1.2 4-1v7.6c-1.5-.2-3 .1-4 1z"/>', 12, 12, 1.2),
