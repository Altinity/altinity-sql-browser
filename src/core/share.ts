...
/** The v2 wire shape's identity-free `query` subset — `id` deliberately
 *  absent (see the module doc above). */
...
const sharedQuery = (query: QueryRoot): SharedQuery => ({
  sql: query.sql,
  specVersion: query.specVersion,
  spec: cloneJson(query.spec),
});

const emptyShared = (): SharedQuery => ({ sql: '', specVersion: SPEC_VERSION, spec: { name: 'Shared query', favorite: false } });

/** Encode a canonical query (or a legacy sql/panel call for compatibility). */
export function encodeShare(queryOrSql: unknown, legacyPanel?: unknown): string {
...
function decodeTagged(obj: unknown): SharedQuery | null {
  if (isPlainObject(obj) && obj.__asb === TAG && isPlainObject(obj.query)) {
  ...
  if (isPlainObject(obj) && obj.__asb === LEGACY_TAG && typeof obj.sql === 'string') {
...
export function decodeShare(hash: string | null | undefined): SharedQuery {
  ...
  if (isPlainObject(parsed) && (parsed.__asb === TAG || parsed.__asb === LEGACY_TAG)) {
