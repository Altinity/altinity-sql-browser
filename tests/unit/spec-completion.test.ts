...
import type { CompleteSpecArgs, SchemaService } from '../../src/core/spec-completion.js';
import {
  createSpecSchemaService, querySpecSchemaService,
} from '../../src/core/spec-schema.js';

// `spec-schema.js` stays untyped `.js` (out of this conversion's scope); its
// exports' TS-inferred shape doesn't line up exactly with `SchemaService`
// (e.g. a `path = []` default infers as `never[]`), so bridge once here.
const asSchemaService = (svc: ReturnType<typeof createSpecSchemaService>): SchemaService => svc as SchemaService;

const complete = (over: Partial<CompleteSpecArgs> = {}) => completeSpec({
  schemaService: asSchemaService(querySpecSchemaService),
...
describe('pure Spec completion', () => {
  ...
  it('offers complete type-specific style keys and values', () => {
    const keys = (type: string) => complete({
      rootValue: { panel: { cfg: { type, style: {} } } }, path: ['panel', 'cfg', 'style'],
    }).map((item) => item.label);
    expect(keys('line')).toEqual(['curve', 'points', 'scale', 'legend', 'grid', 'axes']);
    expect(keys('area')).toEqual(['curve', 'points', 'stack', 'scale', 'legend', 'grid', 'axes']);
    expect(keys('bar')).toEqual(['mode', 'density', 'scale', 'legend', 'grid', 'axes']);
    expect(keys('hbar')).toEqual(['mode', 'density', 'scale', 'legend', 'grid', 'axes']);
    expect(keys('pie')).toEqual(['shape', 'legend', 'frame']);
    const values = (type: string, field: string) => complete({
      rootValue: { panel: { cfg: { type, style: {} } } }, path: ['panel', 'cfg', 'style', field], positionKind: 'property-value',
    }).map((item) => item.insert);
    expect(values('line', 'curve')).toEqual(['"linear"', '"smooth"', '"stepped"']);
    expect(values('line', 'points')).toEqual(['"auto"', '"show"', '"hide"']);
    expect(values('area', 'stack')).toEqual(['"overlay"', '"stacked"']);
    expect(values('bar', 'mode')).toEqual(['"grouped"', '"stacked"']);
    expect(values('bar', 'density')).toEqual(['"normal"', '"compact"', '"joined"']);
    expect(values('pie', 'shape')).toEqual(['"pie"', '"donut"']);
    expect(values('line', 'scale')).toEqual(['"auto"', '"zero"', '"data"']);
    expect(values('line', 'legend')).toEqual(['"auto"', '"show"', '"hide"']);
    expect(values('line', 'grid')).toEqual(['"auto"', '"show"', '"hide"']);
    expect(values('line', 'axes')).toEqual(['"show"', '"hide"']);
    expect(values('pie', 'frame')).toEqual(['"normal"', '"compact"']);
  });
  ...
  it('discovers a synthetic future branch and ranks planned/deprecated variants last', () => {
    ...
    const service = asSchemaService(createSpecSchemaService({ schema, validateCompiled: () => true }));
    expect(complete({
  ...
  it('breaks a full category/order tie (same branch) by label, then by original position', () => {
    // Two enum members of the SAME branch share order/category/deprecated/status,
    // and a number vs. its string twin share a label ('1') too — the only way to
    // reach rank()'s last two tie-breaks (label localeCompare, then position).
    const schema = {
      type: 'object', properties: { cfg: {
        type: 'object', 'x-altinity-discriminator': 'kind', properties: { kind: { type: ['number', 'string'] } },
        oneOf: [{ properties: { kind: { enum: [1, '1'] } } }],
      } },
    };
    const service = asSchemaService(createSpecSchemaService({ schema, validateCompiled: () => true }));
    const items = complete({
    ...
    expect(items.map((item) => [item.label, item.insert])).toEqual([['1', '1'], ['1', '"1"']]);
  });
