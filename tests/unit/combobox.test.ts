...
import type { ComboField, ComboKeyEvent, ComboOption } from '../../src/ui/combobox.js';

function makeParts(inputProps: Record<string, string> = {}) {
...
describe('createCombobox — onClose hook (phase-7 user feedback)', () => {
  function buildWithClose(options: ComboOption[] = PRESETS) {
    const { input, listEl, liveEl } = makeParts();
    const onCommit = vi.fn();
    const onClose = vi.fn();
    const getOptions = vi.fn((text: string) => options.filter((o) => !text || o.value.includes(text) || o.label.includes(text)));
...
describe('wireComboInput (shared field-event wiring — review F8)', () => {
  function wiredField() {
    ...
    const field: ComboField = {
      ...
      onKeyDown: vi.fn((e: ComboKeyEvent) => combo.onKeyDown(e)),
