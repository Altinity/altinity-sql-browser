...
// Every completion this suite applies always has a function `apply` (a plain
// string `apply` — see the "line" option below — is asserted, never called).
function applyCompletion(view: EditorView, completion: Completion, from: number, to: number): void {
...
describe('Spec path source ranges', () => {
  it('maps object keys with dots and array indexes to value nodes', () => {
    ...
    const slice = (path: (string | number)[]) => {
      ...
      // `!`: every path exercised below was just recorded by jsonPathRanges above.
  ...
  it('uses the last duplicate key, matching JSON.parse', () => {
    ...
    // `!`: "x" was just recorded by jsonPathRanges above.
...
describe('Spec editor adapter', () => {
  ...
  it('marks semantic/parse diagnostics, falls back to ancestors, and navigates', () => {
    ...
    // `!`: `setDiagnostics` above always marks a real node for this code.
  ...
  it('builds schema completions from current app state and applies a property in one edit', () => {
    ...
    // `!`: this cursor always resolves a completion result / matching option here.
    ...
    // `as`: this adapter's own `info` is always a plain `() => Node` (ignores
    // its `completion` argument), narrower than CM6's general `Completion.info`.
    const info = panel.info as () => Node;
    expect(info().textContent).toContain('Panel configuration');
    applyCompletion(view, panel, result.from, result.to!);
    expect(port.getValue()).toBe('{\n  "panel": {}\n}');
    expect(changes).toEqual(['{\n  "panel": {}\n}']);
    expect(undo(view)).toBe(true);
    expect(port.getValue()).toBe('{\n  "pa"\n}');
  ...
  it('replaces only an existing key name and inserts a valid root property from an empty document', () => {
    ...
    applyCompletion(view, favorite, result.from, result.to!);
    expect(port.getValue()).toBe('{\n  "favorite": false\n}');
  ...
  it('applies finite values plainly and schema snippets as safe whole-object edits', () => {
    ...
    // `!`: this cursor always resolves a completion result / matching options here.
    ...
    applyCompletion(view, snippet, result.from, result.to!);
    expect(port.getValue()).toBe([
      '{"panel":{"cfg":{',
      '  "type": "line",',
      '  "x": 0,',
  ...
  it('reads result columns/indexes and query parameters only from cached active-tab data', () => {
    const app = makeApp();
    const tab = activeTab(app.state);
    // `Object.assign`: `QueryTab.lastSuccessfulResultColumns` is declared
    // `string[]` (state.ts), but app.js actually assigns column-descriptor
    // objects there (see ui/app.js's post-run snapshot) — out of scope for
    // this leaf conversion (state.ts isn't one of its files); Object.assign's
    // separately-inferred source type sidesteps that mismatched declaration
    // without an unsafe cast.
    Object.assign(tab, {
      lastSuccessfulResultColumns: [
