...
import type { SavedQueryV2, StoredWorkspaceV5, DashboardDocumentV2 } from '../../src/generated/json-schema.types.js';
...
const dashboardSeed = (): StoredWorkspaceV5 => ({
  storageVersion: 5, id: 'w1', key: 'team', name: 'Team', queries: [tiledQuery('q1'), tiledQuery('q2')],
  dashboards: [twoTileDashboard()],
});

/** Load the committed workspace from the shared store (fails loudly if empty). */
async function committed(app: App): Promise<StoredWorkspaceV5> {
...
async function seed(a: App, b: App, workspace: StoredWorkspaceV5): Promise<void> {
...
describe('cross-tab read-before-write (#343)', () => {
  ...
  it('save-linked to an externally deleted query aborts and does not recreate it', async () => {
    const store = fakeIndexedDbFactory();
    const a = tab(store); const b = tab(store);
    const seedNoDash: StoredWorkspaceV5 = {
      storageVersion: 5, id: 'w1', key: 'team', name: 'Team',
...
describe('cross-tab refresh + linked-tab reconcile (#343)', () => {
  const oneQuery = (sql = 'SELECT 1', name = 'q1'): StoredWorkspaceV5 => ({
    storageVersion: 5, id: 'w1', key: 'team', name: 'Team',
  ...
  it('a flagged conflict survives a Library star/rename plus an unrelated external change (#343 review blocker)', async () => {
    const store = fakeIndexedDbFactory();
    const a = tab(store); const b = tab(store);
    const twoQueries: StoredWorkspaceV5 = {
      storageVersion: 5, id: 'w1', key: 'team', name: 'Team',
      queries: [
