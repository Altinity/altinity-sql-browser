...
// #447 deleted the whole `prepareFilterPreview` suite: the Filter-role source
// preview (and `core/filter-execution.js` behind it) no longer exists, so the
// session has no such method to unit-test.
