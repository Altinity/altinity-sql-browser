...
describe('openInDetachedTab — real browser tab', () => {
  ...
  it('mirrors CSS + theme, sets body/title, focuses the tab, and calls mount() with the child doc', () => {
    ...
    const mount = vi.fn<(ctx: MountCtx) => void>();
    openInDetachedTab(app, { title: 'Widget', mode: 'graph', mount });
    expect(qs(win.document, 'style').textContent).toBe('body{color:red}');
    expect(qs(win.document, 'link[rel="icon"]').getAttribute('href')).toBe('data:image/svg+xml;base64,AA');
    ...
    expect(qs(arg.bar, '.graph-overlay-title').textContent).toBe('Widget');
  ...
  it('falls back to stylesText="" when the app has none', () => {
    const win = makeWin();
    openInDetachedTab({ openWindow: () => win, state: detachedState() }, { title: 'X', mode: 'graph', mount: () => {} });
    expect(qs(win.document, 'style').textContent).toBe('');
  });
  ...
  it('tracks two simultaneously-open tabs independently — closing one leaves the other counted', () => {
    const winA = makeWin();
    const winB = makeWin();
    // `openWindow` starts pointed at winA (matches the original "assign right
    // before the first call" JS timing — the property's value at that first
    // call is what matters, not when the assignment happens to run).
    const app: DetachedViewApp = { state: detachedState(), openWindow: () => winA };
    let closeA!: () => void;
...
describe('openInDetachedTab — overlay fallback', () => {
  ...
  it('mounts the overlay in the main document; mount() places closeBtn — placement is not the primitive\'s call', () => {
    ...
    openInDetachedTab(app, {
      ...
      mount: ({ doc, bar, closeBtn }) => {
        ...
        bar.append(ownButton, closeBtn as HTMLElement); // own content first, closeBtn last — mount()'s choice
      },
    });
    const overlay = qs(document, '.graph-overlay');
    expect(overlay).not.toBeNull();
    expect(qs(overlay, '.graph-overlay-title').textContent).toBe('Widget');
  ...
  it('passes a real, close-wired button as closeBtn (mount() can click it directly)', () => {
    ...
    openInDetachedTab({ document, openWindow: () => null, state: detachedState() }, {
      title: 'X', mode: 'graph', mount: ({ closeBtn }) => { received = closeBtn as HTMLElement; },
    });
  ...
  it('a gesture starting inside the panel and ending on the backdrop does not close it (#110)', () => {
    ...
    const overlay = qs(document, '.graph-overlay');
    qs(overlay, '.graph-overlay-canvas').dispatchEvent(new MouseEvent('mousedown', { bubbles: true }));
  ...
  it('the ✕ button closes the overlay; a repeat click on the same (now-detached) backdrop is a no-op', () => {
    const teardown = vi.fn();
    openInDetachedTab({ document, openWindow: () => null, state: detachedState() }, {
      title: 'X', mode: 'graph', mount: ({ bar, closeBtn }) => { bar.appendChild(closeBtn as HTMLElement); return teardown; },
    });
    const overlay = qs(document, '.graph-overlay');
    qs(overlay, '.graph-overlay-close').dispatchEvent(new Event('click', { bubbles: true }));
    expect(document.body.contains(overlay)).toBe(false);
  ...
  it('falls back to overlay when openWindow returns null, a windowless object, or throws (COOP)', () => {
    ...
    document.body.innerHTML = '';
    openInDetachedTab({ document, openWindow: () => ({ document: null }) as DetachedWindowLike, state: detachedState() }, { title: 'X', mode: 'graph', mount: () => {} });
    expect(document.querySelector('.graph-overlay')).not.toBeNull();
    document.body.innerHTML = '';
    openInDetachedTab({ document, openWindow: () => ({ get document(): Document { throw new Error('coop'); } }) as DetachedWindowLike, state: detachedState() }, { title: 'X', mode: 'graph', mount: () => {} });
    expect(document.querySelector('.graph-overlay')).not.toBeNull();
  });
