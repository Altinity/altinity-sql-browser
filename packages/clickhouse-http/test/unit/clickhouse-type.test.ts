...
import type { LiteralArg, TypeNode } from '../../src/index.js';
...
describe('parseClickHouseType — parser', () => {
  ...
  it('parses nested wrappers in both valid orders', () => {
    ...
    expect((parseClickHouseType('Nullable(String)')!.args[0] as TypeNode).name).toBe('String');
    expect((parseClickHouseType('LowCardinality(UInt64)')!.args[0] as TypeNode).name).toBe('UInt64');
  });
  ...
  it('parses positional and named Tuple members', () => {
    expect(parseClickHouseType('Tuple(String, UInt64)')!.args.map((a) => (a as TypeNode).name)).toEqual(['String', 'UInt64']);
  ...
  it('backslash escapes the next character verbatim, not just a delimiter or another backslash', () => {
    ...
    expect((parseClickHouseType("DateTime('a\\xb')")!.args[0] as LiteralArg).value).toBe('axb');
  });
  ...
  it('parses Map and Array', () => {
    ...
    expect((parseClickHouseType('Array(String)')!.args[0] as TypeNode).name).toBe('String');
  });
  ...
  describe('Enum8/Enum16 member parsing', () => {
    ...
    it('a heredoc member wrapped in an outer type still resyncs the token cursor correctly', () => {
      ...
      expect((node.args[0] as TypeNode).enumMembers).toEqual([{ name: 'a,b)', code: 1 }, { name: 'c', code: 2 }]);
    });
...
describe('structural queries', () => {
  ...
  it('arrayElement/mapTypes/namedTupleMembers unwrap Nullable but not LowCardinality (Array/Map/Tuple can never be LowCardinality)', () => {
    expect((arrayElement(parseClickHouseType('Nullable(Array(String))')) as TypeNode).name).toBe('String');
    expect(arrayElement(parseClickHouseType('String'))).toBeNull();
    expect(arrayElement(null)).toBeNull();
    expect((mapTypes(parseClickHouseType('Map(String, Nullable(UInt64))')) as [TypeNode, TypeNode]).map((n) => n.name)).toEqual(['String', 'Nullable']);
