...
// `SchemaApp` (schema.ts) is a narrow slice of the real `App` controller —
// tests/helpers/fake-app.js's `makeApp()` satisfies it directly, same
// convention as tabs.test.ts's use of `FakeApp`.
type FakeApp = ReturnType<typeof makeApp>;

// The schema-tree fixture shape this file builds/mutates on `app.state.schema`
// — schema.ts's own `SchemaDbEntry`/`SchemaTableEntry`/`SchemaColumnEntry`
// types aren't exported (module-private), so this pins the same shape locally
// for the fixtures below. `state.schema` itself stays `Signal<unknown[] | null>`
// (state.ts keeps it deliberately loose), so `dbs()` is the one cast site.
...
interface TestTable {
  ...
  total_bytes: string;
  comment: string;
  columns: TestColumn[] | 'loading' | null;
}
interface TestDb {
  db: string;
  tables: TestTable[];
...
const dbs = (app: FakeApp): TestDb[] => app.state.schema.value as TestDb[];

const rows = (app: FakeApp): HTMLElement[] => [...app.dom.schemaList.querySelectorAll<HTMLElement>('.tree-row')];
// Column rows only ('.tree-row.small'), found by their name label text.
const colRow = (app: FakeApp, name: string): HTMLElement | undefined => [...app.dom.schemaList.querySelectorAll<HTMLElement>('.tree-row.small')]
...
const shiftClick = (el: Element): boolean => el.dispatchEvent(new MouseEvent('click', { bubbles: true, shiftKey: true }));
...
const dblclick = (el: Element): void => { click(el); click(el); };
...
const setExpanded = (app: FakeApp, ...keys: string[]): void => { app.state.expanded.value = new Set([...app.state.expanded.value, ...keys]); };
// Fire a dragstart with a stub dataTransfer and return all setData payloads by MIME.
const dragstart = (el: Element): Record<string, string> => {
  const e: Event & { dataTransfer?: { setData: (mime: string, value: string) => void } } = new Event('dragstart', { bubbles: true });
  const by: Record<string, string> = {};
  e.dataTransfer = { setData: (mime: string, value: string) => { by[mime] = value; } };
  el.dispatchEvent(e);
  by.mime = Object.keys(by)[0]; by.value = by[by.mime]; // back-compat for single-MIME rows
  return by;
};

function withSchema(): FakeApp {
  const app = makeApp();
...
describe('renderSchema states', () => {
  it('no-ops without a list mount', () => {
    const app = makeApp();
    (app.dom as { schemaList: HTMLElement | null }).schemaList = null;
...
describe('renderSchema tree', () => {
  ...
  it('shows the db comment as hover text when present, else the default + drag hint', () => {
    const app = withSchema();
    dbs(app)[1].comment = 'second database';
    renderSchema(app);
  ...
  it('the chevron rotates to the down/open orientation on expand and back on collapse', () => {
    ...
    try {
      ...
      expect((db2Row.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(-90deg)'); // starts collapsed
      click(db2Row); // expand
      expect((db2Row.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(0deg)');
      ...
      expect((db2Row.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(-90deg)');
    } finally {
      vi.useRealTimers();
    }
  });
  ...
  it('expanding a table with no columns triggers loadColumns', () => {
    ...
    renderSchema(app);
    const ordersRow = rows(app).find((r) => r.querySelector('.label')!.textContent === 'orders')!;
    expect((ordersRow.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(-90deg)'); // starts collapsed
    click(ordersRow);
    ...
    expect((ordersRow.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(0deg)');
  });
  it('collapsing an already-loaded table just re-renders', () => {
    const app = withSchema();
    dbs(app)[0].tables[0].columns = [{ name: 'id', type: 'UInt64', comment: '' }];
  ...
  it('shows a loading row while columns load', () => {
    const app = withSchema();
    dbs(app)[0].tables[0].columns = 'loading';
    setExpanded(app, 'tb:db1.orders');
    renderSchema(app);
  ...
  it('columns: a plain click inserts nothing, a quick repeat (double-click) inserts the name', () => {
    ...
    click(colRowEl); // quick repeat → double-click
    expect(app.actions.insertAtCursor).toHaveBeenCalledWith('id');
  });
  it('columns: shift-click inserts name::type', () => {
    const app = withSchema();
    dbs(app)[0].tables[0].columns = [{ name: 'id', type: 'UInt64', comment: 'pk' }];
    setExpanded(app, 'tb:db1.orders');
    renderSchema(app);
    const colRowEl = [...app.dom.schemaList.querySelectorAll<HTMLElement>('.tree-row.small')]
      .find((r) => r.querySelector('.label')!.textContent === 'id')!;
  ...
  it('columns: compacts an unbounded type in the row meta, full type leads the hover title (#177)', () => {
    ...
    setExpanded(app, 'tb:db1.orders');
    renderSchema(app);
    const row = (name: string): HTMLElement => [...app.dom.schemaList.querySelectorAll<HTMLElement>('.tree-row.small')]
      .find((r) => r.querySelector('.label')!.textContent === name)!;
  ...
  it('columns: a type-less column row renders an empty meta and a hint-only title', () => {
    const app = withSchema();
    dbs(app)[0].tables[0].columns = [{ name: 'odd', comment: '' }];
    setExpanded(app, 'tb:db1.orders');
    renderSchema(app);
    const row = [...app.dom.schemaList.querySelectorAll<HTMLElement>('.tree-row.small')]
...
describe('renderSchema drag sources', () => {
  ...
  it('dragging a non-bare name emits the correctly quoted identifier', () => {
    const app = withSchema();
    dbs(app)[0].tables[0].columns = [{ name: 'odd col', type: 'String', comment: '' }];
    setExpanded(app, 'tb:db1.orders');
    renderSchema(app);
  ...
  it('a compact Enum summary drags the complete original declaration, byte-for-byte', () => {
    ...
    dbs(app)[0].tables[0].columns = [{ name: 'operation', type: enumType, comment: '' }];
    setExpanded(app, 'tb:db1.orders');
    renderSchema(app);
...
describe('renderSchema in mobile mode (#126)', () => {
  ...
  function mobileSchema(withCols: boolean): FakeApp {
    const app = withSchema();
    if (withCols) {
      dbs(app)[0].tables[0].columns = [{ name: 'id', type: 'UInt64', comment: 'pk' }];
      setExpanded(app, 'tb:db1.orders');
...
describe('renderSchema with non-bare object names (backtick quoting)', () => {
  ...
  const tbRow = (app: FakeApp): HTMLElement => rows(app).find((r) => r.querySelector('.label')!.textContent === PARQUET)!;
...
describe('renderSchema filter', () => {
  ...
  it('reveals a table when one of its columns matches the filter', () => {
    const app = withSchema();
    dbs(app)[0].tables[1].columns = [{ name: 'user_id', type: 'UInt64', comment: '' }];
...
function withSearchSchema(): FakeApp {
  const app = makeApp();
  app.state.schema.value = [
    {
      db: 'analytics',
...
const colRowsNamed = (app: FakeApp, name: string): HTMLElement[] => [...app.dom.schemaList.querySelectorAll<HTMLElement>('.tree-row.small')]
...
describe('renderSchema search cascade (#208)', () => {
  it('database match: shows the whole matching database, hides the other, columns stay collapsed, expanded state untouched', () => {
    ...
    expect((dbRow.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(0deg)');
    const eventsRow = rows(app).find((r) => r.querySelector('.label')!.textContent === 'events')!;
  ...
  it('table match: shows the parent db, matching table, and all its loaded columns; hides sibling tables', () => {
    ...
    expect((tblRow.querySelector('.chev') as HTMLElement).style.transform).toBe('rotate(0deg)');
  ...
  it('a directly matching table with columns not yet loaded shows no columns and triggers no load', () => {
    const app = withSearchSchema();
    dbs(app)[0].tables[0].columns = null; // events
    app.state.schemaFilter.value = 'events';
    renderSchema(app);
    const labels = rows(app).map((r) => r.querySelector('.label')!.textContent);
    expect(labels).toContain('analytics');
  ...
  it('a directly matching table whose columns are loading shows the loading row', () => {
    const app = withSearchSchema();
    dbs(app)[0].tables[0].columns = 'loading'; // events
    app.state.schemaFilter.value = 'events';
    renderSchema(app);
    expect(app.dom.schemaList.textContent).toContain('loading columns…');
  });
