...
export const DETERMINISTIC_SCENARIOS: DeterministicScenario[] = [
  ...
  { id: 'read-reset-retries-once', description: 'a read retries exactly once after a mid-stream connection reset and then succeeds — hand-driven against the shim, same policy shape as the SESSION_IS_LOCKED case (executeScript always mints its own query_id, so it cannot itself be routed to a specific fixture)', fixture: 'read-reset-then-success', invariantRefs: ['Retry safety remains unchanged'] },
