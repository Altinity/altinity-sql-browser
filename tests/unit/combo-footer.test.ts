...
function fakeCombo(open: boolean) {
  return { isOpen: () => open };
}

describe('attachComboFooter', () => {
  it('starts hidden', () => {
    const { listEl } = makeParts();
    const { el } = attachComboFooter({ listEl, combo: fakeCombo(false), hasRecents: () => true, onClear: vi.fn() });
    expect(el.hidden).toBe(true);
    expect(el.classList.contains('var-combo-footer')).toBe(true);
  });
  it('sync() hides when the combobox is closed, even with recents', () => {
    const { listEl } = makeParts();
    const { el, sync } = attachComboFooter({ listEl, combo: fakeCombo(false), hasRecents: () => true, onClear: vi.fn() });
    sync();
    expect(el.hidden).toBe(true);
  });
  it('sync() hides when open but there are no recents for this field', () => {
    const { listEl } = makeParts();
    const { el, sync } = attachComboFooter({ listEl, combo: fakeCombo(true), hasRecents: () => false, onClear: vi.fn() });
    sync();
    expect(el.hidden).toBe(true);
  });
  it('sync() shows and positions the footer when open with recents', () => {
    const { listEl } = makeParts();
    const { el, sync } = attachComboFooter({ listEl, combo: fakeCombo(true), hasRecents: () => true, onClear: vi.fn() });
    sync();
  ...
  it('the Clear button has tabindex -1 (never steals Tab focus) and a stable label', () => {
    const { listEl } = makeParts();
    const { el } = attachComboFooter({ listEl, combo: fakeCombo(true), hasRecents: () => true, onClear: vi.fn() });
  ...
  it('clicking Clear preventDefaults the mousedown (stays open, no blur), calls onClear, and re-syncs', () => {
    ...
    const { el, sync } = attachComboFooter({ listEl, combo: fakeCombo(true), hasRecents: () => hasRecents, onClear });
    sync();
    expect(el.hidden).toBe(false);
    const btn = el.querySelector('button.var-combo-clear')!;
