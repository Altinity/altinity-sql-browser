...
import type { SpecValidatorArgs, SpecValidatorEntry } from '../../src/core/spec-draft.js';
...
describe('Spec semantic validation', () => {
  ...
  it('supports registered exact array paths and dotted property keys', () => {
    ...
    const validate = vi.fn(({ value, path, root, present }: SpecValidatorArgs) => {
      const r = root as Record<string, { kind: string }[]>;
      expect(r['a.b'][0].kind).toBe('bad');
  ...
  it('normalizes validator defaults and missing paths deterministically', () => {
    const validators: SpecValidatorEntry[] = [{ path: ['missing'], validate: (ctx: SpecValidatorArgs) => {
...
describe('Spec draft evaluation/formatting', () => {
  ...
  it('accepts a bare validator-entry array in place of a service object', () => {
    // Same `readonly SpecValidatorEntry[]` alternative `validateSpec` itself
    // accepts — evaluateSpecText wraps it through `createQuerySpecValidationService`
    // rather than requiring callers to build a full registry object first.
    const arrayValidators: SpecValidatorEntry[] = [];
    expect(evaluateSpecText('{"favorite":"yes"}', arrayValidators)).toEqual({
      parsed: { favorite: 'yes' },
      diagnostics: [{
        path: ['favorite'], severity: 'error', code: 'schema-invalid-type',
        message: 'favorite must be boolean', keyword: 'type', schemaId: SPEC_SCHEMA_ID,
  ...
  it('accepts an app-owned validator registry', () => {
    const registry = createSpecValidatorRegistry([]);
    registry.register(['items', 0, 'kind'], ({ value }: SpecValidatorArgs) => value === 'ok' ? [] : [{ message: 'bad kind' }]);
