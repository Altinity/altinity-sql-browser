...
/** One recorded recent value for a variable — `seq` is the strictly-
 *  increasing global counter that gives one true recency order across every
 *  name (see the module doc above). */
 ...
 *  the same shape `state.ts`'s `RecentMap` pins for this (previously
 *  unconverted) module's result. */
...
export function emptyRecentMap(): RecentMap {
  return { version: 1, nextSeq: 1, byName: {} };
}

function asMap(map: RecentMap | null | undefined): RecentMap {
...
function enforceTotalCap(map: RecentMap): RecentMap {
  ...
  const flat: { name: string; seq: number }[] = [];
  ...
  const removeSeqs = new Map<string, Set<number>>(); // name -> Set(seq)
 ...
 * mutates `map`.
 */
export function recordRecent(map: RecentMap | null | undefined, name: string, value: string | null | undefined): RecentMap {
...
export function clearAllRecent(): RecentMap {
...
export function filterRecentValues(list: string[], text: string | null | undefined): string[] {
...
export function recentOptions(map: RecentMap | null | undefined, name: string, type: string, text: string | null | undefined): string[] {
