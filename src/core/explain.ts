...
/** One EXPLAIN result-pane view definition — see `EXPLAIN_VIEWS` below. */
export interface ExplainViewDef {
  id: string;
  label: string;
  kind: 'text' | 'table' | 'graph';
  chFormat: 'TabSeparatedRaw' | 'Table';
}
 ...
 */
export const EXPLAIN_VIEWS: ExplainViewDef[] = [
...
/** `parseExplain`'s return shape — `null` when `sql` is not an EXPLAIN. */
 ...
 */
export function parseExplain(sql: unknown): ParsedExplain | null {
  const s = String(sql || '');
 ...
 */
export function detectExplainView(parsed: ParsedExplain | null): string | null {
  if (!parsed) return null;
  const set = parsed.settings || {};
  const keys = Object.keys(set);
  const onlyOne = (k: string): boolean => keys.length === 1 && set[k] === '1';
...
/** Options `buildExplainQuery` accepts — see below. */
export interface BuildExplainQueryOpts {
  pretty?: boolean;
}
 ...
 */
export function buildExplainQuery(inner: unknown, viewId: string, opts: BuildExplainQueryOpts = {}): string {
