...
/** A Filter favorite's curated option (a subset of `ComboOption` — no `group`). */
...
/** What `buildFilterOptionField` returns. */
...
export function buildFilterOptionField({
  ...
  const selected = (): FilterFieldOption | undefined => options.find((option) => option.value === value);
  const input = h('input', {
