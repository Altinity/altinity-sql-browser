...
export function codePresentationExtensions(): Extension[] {
  return [
    lineNumbers(),
    drawSelection(),
...
export function createWrapCompartment(enabled = false): { extension: Extension; reconfigure: (wrap: boolean) => StateEffect<unknown> } {
  const compartment = new Compartment();
  const value = (wrap: boolean): Extension => (wrap ? EditorView.lineWrapping : []);
  return {
    extension: compartment.of(value(enabled)),
    reconfigure: (wrap: boolean) => compartment.reconfigure(value(wrap)),
  };
}
