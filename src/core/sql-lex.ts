...
export type TokenKind = SpanKind | 'word' | 'number' | 'punct' | 'op' | 'other';

/** One flat token `lexSql` yields — see its doc comment for the contract. */
...
const isWordStart = (c: string): boolean => (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z') || c === '_';
...
const isWordPart = (c: string): boolean => isWordStart(c) || isDigit(c) || c === '$';
const isNumPart = (c: string): boolean => isDigit(c) || c === '.' || c === 'e' || c === 'E' || c === '+' || c === '-';
...
function lexCode(s: string, start: number, end: number, out: Token[]): void {
  let i = start;
  while (i < end) {
    const c = s[i];
 ...
 */
export function lexSql(sql?: string | null): Token[] {
  const s = String(sql || '');
  const out: Token[] = [];
...
export function tokenText(sql: string, token: Token): string {
...
export function isWord(sql: string, token: Token | null | undefined, word: string): boolean {
 ...
 * Pure.
 */
export function decodeQuotedIdent(raw: string, closed = true): string {
  const s = String(raw);
  const q = s[0];
 ...
 */
export function unquoteIdent(sql: string, token: Token): string {
