// Originally issue #585 Phase 0 (plan §15 "Deterministic protocol-fault
// server"), moved to this package by issue #630 Phase 8 (plan §15 "Reuse
// fault-server.mjs"): it is generic, dependency-free, deterministic HTTP
// fixture infrastructure with no vendor-specific logic at all, so it belongs
// to `@altinity/clickhouse-http`'s own first-party browser regression suite
// (`test/browser/regression.spec.js`) now, not the retired vendor-comparison
// spike.
//
// A tiny, dependency-free Node `http` server exposing named fixture routes,
// each with a fully deterministic byte sequence and timing — so a real-browser
// regression suite can prove exact transport behavior (progressive first row,
// mid-stream exception detection, malformed/truncated handling, auth retry,
// raw byte exactness, cancellation) WITHOUT depending on real ClickHouse
// timing or scheduling. This file only produces bytes, it never asserts.
//
// Root SQL Browser browser integration (`tests/e2e/export-post-header-
// cancel.spec.js`, `tests/e2e/authenticated-clickhouse-request.spec.js`) also
// imports this package-owned fixture directly while the workspace exists —
// #639's extraction handoff lists that consumer-side path as one to update
// when the workspace is removed (`docs/clickhouse-http-repository-
// extraction.md`).
//
// Kept as plain `.mjs` (not `.ts`): Node orchestration/configuration files
// stay untyped so this package doesn't need a `@types/node` devDependency
// just for this one fixture server.
