...
import type { MdHeadingBlock, MdListBlock, MdParagraphBlock, MdTextNode } from '../../src/core/markdown-lite.js';

const text = (s: string): MdTextNode => ({ t: 'text', text: s });
...
describe('parseMarkdown — inline', () => {
  const inline = (s: string) => (parseMarkdown(s)[0] as MdParagraphBlock).children;
  ...
  it('inline formatting works inside headings and list items', () => {
    expect((parseMarkdown('# a **b**')[0] as MdHeadingBlock).children[1].t).toBe('strong');
    expect((parseMarkdown('- has `code`')[0] as MdListBlock).items[0][1]).toEqual({ t: 'code', text: 'code' });
  });
});
