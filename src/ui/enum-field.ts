...
/** `filterEnumValues`'s result — the filtered/capped options plus enough to
 *  render the "type to narrow" hint. */
export interface FilterEnumValuesResult {
  options: string[];
  total: number;
  truncated: boolean;
}
 ...
 */
export function filterEnumValues(values: string[], text: string | undefined): FilterEnumValuesResult {
...
/** `buildEnumField`'s options bag. */
export interface BuildEnumFieldOpts {
  document?: Document;
  name: string;
  type: string;
  value: string;
