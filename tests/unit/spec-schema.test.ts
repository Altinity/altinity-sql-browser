...
import type { FeatureValidatorFn } from '../../src/core/spec-schema.js';
import type { JsonSchemaValidatorFn, SchemaValidationError } from '../../src/core/json-schema-validation.js';
...
describe('schema lookup', () => {
  ...
  it('supports the documented lookup subset on synthetic schemas', () => {
    ...
    (schema.properties.choice.anyOf as unknown[]).push({ allOf: [{ properties: { extension: { type: 'boolean' } } }] });
    const root = {
  ...
  it('rejects invalid, remote, unresolved, and cyclic lookup schemas', () => {
    ...
    const look = (schema: unknown) => createSpecSchemaService({ schema, validateCompiled: () => true })
...
describe('diagnostic normalization', () => {
  it('maps every stable keyword code and preserves exact pointer segments', () => {
    const compiled: JsonSchemaValidatorFn = () => false;
    compiled.errors = [
    ...
    ].map(([keyword, instancePath, params, message]) => ({ keyword, instancePath, params, message, schemaPath: '#' })) as SchemaValidationError[];
...
describe('feature validation service', () => {
  it('runs registered validators with context and unregisters idempotently', () => {
    ...
    const validator = vi.fn<FeatureValidatorFn>(({ root, path, value, present, context }) => {
      expect((root as { items: { kind: string }[] }).items[0].kind).toBe('bad');
      expect(path).toEqual(['items', 0, 'kind']);
      expect(value).toBe('bad');
      expect(present).toBe(true);
      expect((context as { resultColumns: unknown }).resultColumns).toEqual([{ name: 'x' }]);
