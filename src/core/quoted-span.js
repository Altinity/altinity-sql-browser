...
/**
 * @param {string} text
 * @param {number} start index of the OPENING delimiter itself
 * @param {string} quote the delimiter character (`'`, `` ` ``, or `"`)
 * @returns {{end: number, closed: boolean}} `end` is exclusive — one past the
 *   closing delimiter, or `text.length` when unclosed.
 */
export function scanDelimited(text, start, quote) {
  const n = text.length;
  let j = start + 1;
  while (j < n) {
    const c = text[j];
