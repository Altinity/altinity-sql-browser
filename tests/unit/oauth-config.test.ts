...
import type { IdpDescriptor } from '../../src/net/oauth-config.js';
...
const resp = (ok: boolean, body: unknown, status = ok ? 200 : 500): FakeResponse => ({
  ok,
  status,
  json: async () => body,
});
...
function fetcher(map: [RegExp, FakeResponse][]) {
...
describe('loadConfigDoc', () => {
  const docOf = async (body: unknown, base = '/sql') =>
    (await loadConfigDoc(asFetch(fetcher([[/config\.json$/, resp(true, body)]])), base)).idps;
  ...
  it('wraps a single bare-object config into one IdP (host id/label defaults)', async () => {
    ...
    const { idps } = await loadConfigDoc(asFetch(f), '/sql');
    expect(idps).toEqual([{
  ...
  it('throws when config.json is not ok', async () => {
    ...
    await expect(loadConfigDoc(asFetch(f), '/sql')).rejects.toThrow('config.json: 404');
  });
  ...
  it('defaults basicLogin to true and honours an explicit false', async () => {
    const load = (body: unknown) => loadConfigDoc(asFetch(fetcher([[/config\.json$/, resp(true, body)]])), '/sql');
...
describe('resolveIdp', () => {
  ...
  } as IdpDescriptor; // fixture omits basicUserClaim — not read by resolveIdp
  it('adds authUri/tokenUri from OIDC discovery, preserving the descriptor', async () => {
    ...
    const cfg = await resolveIdp(asFetch(f), idp);
  ...
  it('throws when discovery is not ok', async () => {
    ...
    await expect(resolveIdp(asFetch(f), idp)).rejects.toThrow('OIDC discovery failed: 500');
  });
  it('throws when discovery lacks endpoints', async () => {
    const f = fetcher([[/openid-configuration$/, resp(true, { authorization_endpoint: 'a' })]]);
    await expect(resolveIdp(asFetch(f), idp)).rejects.toThrow('missing authorization_endpoint or token_endpoint');
  });
});
